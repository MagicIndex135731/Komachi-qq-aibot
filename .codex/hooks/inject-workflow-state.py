...
def _codex_mode_banner(config: dict) -> str:
    ...
    `global-policy` mode explicitly leaves dispatch decisions to higher-priority
    Codex/user instructions; Trellis only supplies task state and context.
    """
    mode = "global-policy"
    if isinstance(config, dict):
        ...
        if isinstance(codex_cfg, dict):
            ...
            if cfg_mode in ("global-policy", "inline", "sub-agent"):
                mode = cfg_mode
    if mode == "sub-agent":
        meaning = (
...
def resolve_breadcrumb_key(
    ...
    Codex defaults to ``global-policy``. That mode uses dedicated neutral
    breadcrumb blocks which provide task state without forcing or forbidding
    sub-agent dispatch.
