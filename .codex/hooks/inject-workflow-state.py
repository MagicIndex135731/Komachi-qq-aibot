...
def _resolve_codex_dispatch_mode(config: dict) -> str:
    """Normalize `codex.dispatch_mode` to global-policy, auto, or inline.

    Defaults to `global-policy`. The legacy `sub-agent` value remains an alias
    for `auto`. Any other explicit value (including invalid ones) falls back to `inline`
...
def _codex_mode_banner(config: dict) -> str:
    ...
    Reads `codex.dispatch_mode` from .trellis/config.yaml. The default
    `global-policy` mode leaves dispatch decisions to higher-priority
    Codex/user instructions while preserving Trellis task/context injection.
    `auto` dispatches Trellis sub-agents using native Codex context
    ...
    if mode == "global-policy":
        meaning = (
            "global-policy: Trellis does not decide whether, how, or which "
            "sub-agents to dispatch; follow higher-priority Codex/user instructions."
        )
    elif mode == "auto":
        meaning = (
...
def resolve_breadcrumb_key(
    ...
    Codex defaults to ``global-policy`` and uses a neutral
    ``<status>-global-policy`` breadcrumb. ``auto`` uses the ordinary ``<status>``
