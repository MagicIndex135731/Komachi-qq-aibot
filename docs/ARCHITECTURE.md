...
群聊生图复用聊天使用的 Nova Responses 中转（`LLM_BASE_URL`、`LLM_API_KEY`）。每个生图请求先交给 `gpt-5.6-luna`（low reasoning）判断是否确实需要联网参考图；需要时再把人物拆成独立搜图关键词，由 DDGS 获取参考图，最后使用独立的 `GROUP_IMAGE_MODEL=gpt-5.6-sol` 通过 `image_generation` 工具请求文生图和参考图生图。模型判断失败时，仅对用户明确提出的搜图请求启用本地兜底，不会让普通生图误触发搜索；尺寸默认 `auto`、质量默认 `high`。`GroupImageGenerationService` 负责队列容量、超时、搜索决策、参考图、输出文件和 QQ 发送。旧的 `GROUP_IMAGE_*` endpoint 配置仍保留给未注入具体聊天客户端的兼容调用方。
...
## 11.5 按群记忆策略与问法鲁棒性（2026-08-07 更新）
