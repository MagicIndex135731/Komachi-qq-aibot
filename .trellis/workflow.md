...
<!--
  ...
  TAG ↔ PHASE scoping:
    ...
    [workflow-state:planning-global-policy] → Codex policy-neutral Phase 1
    [workflow-state:in_progress]  → Phase 2 + Phase 3.2-3.4
                                    (status stays 'in_progress' from
                                    task.py start until task.py archive)
    [workflow-state:in_progress-inline] → Codex inline variant of Phase 2/3
    [workflow-state:in_progress-global-policy] → Codex policy-neutral Phase 2/3
...
[workflow-state:planning-global-policy]
Load `trellis-brainstorm`; stay in planning.
Lightweight: `prd.md` can be enough. Complex: finish `prd.md`, `design.md`, and `implement.md`; ask for review before `task.py start`.
Trellis persists task artifacts but does not decide sub-agent use, model, reasoning effort, or parallelism. Follow higher-priority Codex/user instructions.
JSONL manifests are optional in this mode and are not a start gate.
[/workflow-state:planning-global-policy]
...
[workflow-state:in_progress-global-policy]
Continue implementation, checking, validation, spec update, commit, and `/trellis:finish-work` in order.
Trellis supplies task state and persisted context only. Decide whether to work directly or dispatch sub-agents exclusively from higher-priority Codex/user instructions and available tools.
Any dispatched agent must receive the active task path and read `prd.md`, optional `design.md` / `implement.md`, and relevant specs/research.
[/workflow-state:in_progress-global-policy]
...
- Planning or unclear requirements -> `trellis-brainstorm`.
- For implementation, research, and checking, choose direct work or sub-agents only from higher-priority Codex/user instructions and available tools.
- Trellis does not select the model, reasoning effort, number of agents, or parallelism.
- Repeated debugging -> `trellis-break-loop`; spec updates -> `trellis-update-spec`.
...
Choose direct research or a research sub-agent according to higher-priority Codex/user instructions. If dispatching, start the prompt with `Active task: <task path>` and require output under `{TASK_DIR}/research/`.
...
JSONL manifests are optional context aids, not a task-start gate. Curate them when the selected sub-agent workflow benefits from injected spec/research paths; otherwise load task artifacts and specs directly.
...
Choose direct implementation or a sub-agent according to higher-priority Codex/user instructions. In either case, load `prd.md`, optional `design.md` / `implement.md`, and relevant specs/research first. If dispatching, start the prompt with `Active task: <task path>` and state the role boundary explicitly.
...
Choose direct checking or a check sub-agent according to higher-priority Codex/user instructions. The check must cover task artifacts, specs, tests, and the full changed scope; if dispatching, start the prompt with `Active task: <task path>`.
...
| Codex global-policy Phase 1 | `[workflow-state:planning-global-policy]` |
...
| Codex global-policy Phase 2 + Phase 3.2–3.4 | `[workflow-state:in_progress-global-policy]` |
