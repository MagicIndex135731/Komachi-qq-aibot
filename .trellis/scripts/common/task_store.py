...
# Codex is checked separately because global-policy and inline modes do not
# make JSONL a required task-start gate. Kilo / Antigravity / Devin are NOT in
# this list either: they load
...
def _has_subagent_platform(repo_root: Path) -> bool:
    ...
    ``sub-agent``. Global-policy mode does not make JSONL manifests a task
    start gate because dispatch is decided outside Trellis.
    """
