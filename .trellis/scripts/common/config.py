...
DEFAULT_CODEX_DISPATCH_MODE = "global-policy"
...
def get_codex_dispatch_mode(repo_root: Path | None = None) -> str:
    ...
    Default is ``global-policy``. In that mode Trellis persists task context
    but leaves all sub-agent dispatch decisions to higher-priority Codex/user
    instructions. ``inline`` and ``sub-agent`` remain compatibility modes.
    """
    config = _load_config(repo_root)
    ...
    if mode in ("global-policy", "inline", "sub-agent"):
        return mode
    print(
