...
def resolve_effective_platform(platform: str, config: dict) -> str:
    ...
    When ``--platform codex`` is passed, return ``"codex-global-policy"``
    (default), ``"codex-inline"``, or ``"codex-sub-agent"`` based on
    ``.trellis/config.yaml`` ``codex.dispatch_mode``.
    ...
    Global-policy mode keeps Trellis responsible for task persistence while
    leaving dispatch, model, reasoning effort, and parallelism to the active
    higher-priority Codex/user instructions.
