...
- `.trellis/config.yaml` is the project-level Trellis configuration entry point. It hosts task lifecycle hooks (`hooks.after_create` / `after_start` / `after_finish` / `after_archive`), journal shape (`session_commit_message` / `max_journal_lines` / `session_auto_commit`), channel worker guard (`channel.worker_guard.idle_timeout` / `max_live_workers`), Codex dispatch mode (`codex.dispatch_mode: global-policy | auto | inline | sub-agent`), and the spec registry block (`registry.spec.source` + `registry.spec.template`). `global-policy` is the default and leaves dispatch/model/reasoning/parallelism decisions to higher-priority Codex/user instructions.
