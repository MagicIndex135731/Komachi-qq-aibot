...
- `MEMORY_ADAPTIVE_CONTEXT_ENABLED=true` 让 Memory V3 在有效输入 token 总预算内动态分配近期与历史空间；`MEMORY_ADAPTIVE_MAX_RECENT_MESSAGES=60` 和 `MEMORY_ADAPTIVE_MAX_HISTORY_MESSAGES=300` 是应急行数上限，不代表每次固定注入这些消息。@/点名回复与主动插话都以这 60 条消息构建近期上下文；没有当前或引用图片时，二者最多补充最新 2 张群内图片。
